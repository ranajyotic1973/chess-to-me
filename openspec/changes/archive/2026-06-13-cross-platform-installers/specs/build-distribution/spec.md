...
A workflow file at `.github/workflows/release.yml` SHALL trigger on push of a tag matching `v*.*.*` and run three parallel jobs: `build-windows`, `build-mac`, and `build-linux`. Each job SHALL build the respective platform artifacts, rebuild native modules for all target architectures, and upload the resulting installer files to a GitHub Release draft.

#### Scenario: Version tag triggers multi-platform build
- **WHEN** a git tag `v1.2.3` is pushed to the repository
- **THEN** the release workflow SHALL start automatically and all three platform jobs SHALL run in parallel

#### Scenario: All artifacts are attached to the GitHub Release
- **WHEN** all CI jobs complete successfully
- **THEN** the GitHub Release draft SHALL contain installer files for Windows (x64, arm64), macOS (universal), and Linux (x64 and arm64 AppImage + deb)

#### Scenario: CI failure blocks release publication
- **WHEN** any platform build job fails
- **THEN** the GitHub Release SHALL remain in draft state and SHALL NOT be automatically published
...
#### Scenario: Cross-platform builds are documented
- **WHEN** a developer reads the project README or build documentation
- **THEN** they SHALL find a clear note that building for a non-host platform (e.g., macOS from Windows) is only supported via the CI pipeline
