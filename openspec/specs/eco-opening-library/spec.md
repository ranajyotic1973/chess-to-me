...
### Requirement: Live ECO detection in Analysis mode
When the user makes moves in Analysis mode, the renderer SHALL call the `eco:lookup-fen` IPC handler with the current FEN after each board position change. The returned `{ eco, name }` SHALL be displayed below the board as an italic caption. The label SHALL clear when the position is at the starting FEN or when no ECO match is found.

#### Scenario: Known opening position is detected live
- **WHEN** the user plays 1.e4 e5 2.Nf3 Nc6 3.Bb5 in Analysis mode
- **THEN** the opening label SHALL show "Ruy Lopez (C60)" (or the closest named variation) below the board without requiring an explicit user action

#### Scenario: Position leaves the opening book
- **WHEN** the user plays deep into the middlegame where no ECO entry exists
- **THEN** the opening label SHALL be cleared; the last-known opening name SHALL NOT persist

#### Scenario: Starting position shows no label
- **WHEN** the board is at the standard starting position
- **THEN** no opening label SHALL be displayed
