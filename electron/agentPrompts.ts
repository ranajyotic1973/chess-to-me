...
// BUG FIX (2026-07-03): The LLM was previously analyzing the next move in the line
// instead of analyzing the current position. This has been fixed by adding explicit
// constraints to the system prompt telling the LLM to analyze ONLY the current board
// state (as defined by the FEN and moves already played), and to never predict or
// reference future moves. This ensures analysis is based on moves the user has actually
// made, not moves they might make next.
...
CRITICAL CONSTRAINT: Analyze ONLY the board position shown by the current FEN and the moves already played. Do NOT predict, analyze, or reference moves beyond what has already been played. Focus strictly on the current position.
...
**Current Position Analysis**: [Brief assessment of the position after the moves that have been played]
...
**Opening Name** (if applicable): [If the current position is at move 3 or beyond, identify and name the chess opening (e.g., "This is the Sicilian Defense, Najdorf Variation"). Only include this section if the opening is clear enough to identify.]
...
Guidelines:
- MANDATORY: Analyze ONLY the current position created by the moves provided. Do NOT predict future moves.
