...
async function createWindow(): Promise<void> {
  try {
    ...
    logToFile("DEBUG", "createWindow", "Creating BrowserWindow", {
    ...
    logToFile("DEBUG", "createWindow", "BrowserWindow created");
    ...
    if (devUrl) {
      logToFile("DEBUG", "createWindow", "Loading dev URL", { url: devUrl });
      await win.loadURL(devUrl);
    } else {
      const indexPath = path.join(__dirname, "..", "..", "dist", "index.html");
      logToFile("DEBUG", "createWindow", "Loading file", { path: indexPath });
      await win.loadFile(indexPath);
    }

    logToFile("INFO", "createWindow", "Content loaded");
    ...
    logToFile("ERROR", "createWindow", "Failed to create window", { error: msg });
    throw err;
  }
}
...
async function doImportGamesFile(
  ...
  const extractDir = opts.extractDir ?? defaultExtractDir;
  ...
  try {
    ...
    // Use provided shared DB, or open our own
    ...
    for (let i = 0; i < pgnFiles.length; i++) {
      total += await importPgnFile(db, pgnFiles[i], (n) =>
    ...
    if (!opts.skipFts) {
      ...
      setGamesSource(db, "Lumbra's Gigabase");
      rebuildFts(db);
      db.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('fts_built', 'true')").run();
      db.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('last_update_import', ?)").run(new Date().toISOString().slice(0, 10));
    }

    if (lower.endsWith(".7z") && fs.existsSync(extractDir)) {
      fs.rmSync(extractDir, { recursive: true, force: true });
    }
...
ipcMain.handle("db:import-otb-dir", async (event, { dirPath }: { dirPath: string }) => {
  ...
  (async () => {
    ...
    // Open one shared DB connection for the whole batch
    if (gamesDb) { gamesDb.close(); gamesDb = null; }
    gamesDb = initGamesDb(gamesDbPath);
    ...
    if (imported > 0) {
      ...
      try {
        ...
        rebuildFts(sharedDb);
        sharedDb.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('fts_built', 'true')").run();
        sharedDb.prepare("INSERT OR REPLACE INTO meta (key, value) VALUES ('last_update_import', ?)").run(new Date().toISOString().slice(0, 10));
      } catch (ftsErr) {
...
app.whenReady().then(async () => {
  ...
  logToFile("DEBUG", "main", "Registering IPC handlers");
  registerIpcHandlers();

  logToFile("DEBUG", "main", "Setting application menu");
  Menu.setApplicationMenu(null);

  logToFile("DEBUG", "main", "Creating window");
  await createWindow();
  ...
  initEcoLookup(getBundledEcoDataDir()).catch(err => {
    const msg = err instanceof Error ? err.message : String(err);
    logToFile("WARN", "main", "ECO lookup startup init failed", { error: msg });
  });

  // Show monthly update prompt after the window has fully loaded (30-day throttle)
  if (mainWindow) {
    mainWindow.webContents.once("did-finish-load", () => {
      logToFile("INFO", "main", "Window did-finish-load");
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    logToFile("ERROR", "main", "ProcessManager init error", { error: msg });
  }
